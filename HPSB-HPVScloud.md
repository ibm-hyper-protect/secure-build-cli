...
- To update the hostname, or to update the instance with a new certificate when the old certificate expires, complete the following steps:
  ...
  6. Run the following command to update the HPSB instance (in the case of certificate expiration, you need not update the hostname):
     ```buildoutcfg
     ibmcloud hpvs instance-update SBContainer --rd-path secure_build.asc -i 1.3.0.12 --hostname sbs.example.com -e CLIENT_CRT=... -e CLIENT_CA=... -e SERVER_CRT=... -e SERVER_KEY=...
     ```
...
- 1.3.0.12 is the image tag of Secure Docker Build docker image.
...
2. Modify the `sbs-config.json` file for 1.3.0.12 according to the following instructions:
...
```buildoutcfg
ibmcloud hpvs instance-update HPSBContainer -i 1.3.0.12 --rd-path "secure_build.asc" --hostname="sbs.example.com" -e CLIENT_CRT=... -e CLIENT_CA=... -e SERVER_CRT=... -e SERVER_KEY=...
```
