...
IMAGE_TAG - image tag of the container image to be deployed as HPSB server. Use "1.3.0.15" unless otherwise noted.
...
- To update the hostname, or to update the instance with a new certificate when the old certificate expires, complete the following steps:
  ...
  6. Run the following command to update the HPSB instance (in the case of certificate expiration, you need not update the hostname):
     ```buildoutcfg
     ibmcloud hpvs instance-update SBContainer --rd-path secure_build.asc -i 1.3.0.15 --hostname sbs.example.com -e CLIENT_CRT=... -e CLIENT_CA=... -e SERVER_CRT=... -e SERVER_KEY=...
     ```
...
- 1.3.0.15 is the image tag of Secure Docker Build docker image.
...
**Important:** If you are updating from HPSB version earlier than 1.3.0.9 to 1.3.0.15, complete the instructions from steps 1 to 4. Otherwise, you can start from step 5.
...
2. Modify the `sbs-config.json` file for 1.3.0.15 according to the following instructions:
...
```buildoutcfg
ibmcloud hpvs instance-update HPSBContainer -i 1.3.0.15 --rd-path "secure_build.asc" --hostname="sbs.example.com" -e CLIENT_CRT=... -e CLIENT_CA=... -e SERVER_CRT=... -e SERVER_KEY=...
```
...
Last image update     2024-07-09 05:13
Created               2024-07-08
```
